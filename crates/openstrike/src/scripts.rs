...
impl Headless {
    ...
    pub fn shot(&mut self, game: &mut OpenStrike, time: f32, path: &str) -> Result<()> {
        self.shot_with_hud(game, None, time, path)
    }

    /// Screenshot with the JSX HUD composited (round/lose scripts carry the
    /// live guest).
    pub fn shot_with_hud(
        &mut self,
        game: &mut OpenStrike,
        guest: Option<&mut StrikeGuest>,
        time: f32,
        path: &str,
    ) -> Result<()> {
        ...
        if let Some(guest) = guest {
            ...
            guest.render_overlay(&self.gpu, &mut encoder, &self.target.view, OFFSCREEN_FORMAT)?;
...
fn place_bots_near_player(game: &mut OpenStrike, count: usize) {
    ...
    for i in 0..count {
        ...
        bot.anim.clip = game.bot_walk_clip;
        game.bots.push(bot);
    }
}
...
fn round_script(mut game: OpenStrike, args: &Args) -> Result<()> {
    ...
    // The gameplay mod (JS) owns phase transitions; boot the guest.
    ...
    for tick_no in 0..budget {
        ...
        if win_seen && matches!(game.phase, Phase::Starting) {
            restart_seen = true;
            if let Some(b) = &base {
                hl.shot_with_hud(
                    &mut game,
...
fn lose_script(mut game: OpenStrike, args: &Args) -> Result<()> {
    ...
    for tick_no in 0..budget {
        ...
        if lose_seen && matches!(game.phase, Phase::Starting) {
            break;
        }
    }
...
fn walk_script(mut game: OpenStrike, args: &Args) -> Result<()> {
    ...
    // 2b) Viewmodel anti-jitter invariant: while moving at full speed, the
    // gun's offset from the interpolated camera eye must be identical at
    // every render alpha — a tick-anchored gun would drift by
    // vel * dt * (1 - alpha) (~2 units at 250 u/s) and visibly shake.
    {
        let offsets: Vec<Vec3> = [0.0f32, 0.25, 0.5, 0.75, 1.0]
            .iter()
            .map(|&a| {
                let gun = game.viewmodel_transform_at(a).transform_point3(Vec3::ZERO);
                gun - game.player.eye_interpolated(a)
            })
            .collect();
        let max_drift = offsets[1..]
            .iter()
            .map(|o| (*o - offsets[0]).length())
            .fold(0.0f32, f32::max);
